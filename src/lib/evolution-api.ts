import { ApiConfig } from "@/contexts/ApiConfigContext";
    ...
    interface ChatHistoryResponse {
        messages: WhatsappMessage[]; // Assuming WhatsappMessage type is defined elsewhere or here
    }
    ...
    export const sendMessage = async (apiConfig: ApiConfig, jid: string, text: string): Promise<MessageSendResponse> => {
        ...
        console.log(`[EvolutionAPI] Sending Text: POST ${url}`);
        ...
        try {
            ...
            console.error("[EvolutionAPI] Error in sendMessage:", error);
            if (error instanceof EvolutionApiError) throw error;
     ...
     */
    export const fetchChatHistory = async (apiConfig: ApiConfig, jid: string, limit: number = 100): Promise<WhatsappMessage[]> => {
        const apiUrl = getFormattedApiUrl(apiConfig.apiUrl);
        if (!apiUrl || !apiConfig.apiKey || !apiConfig.apiInstance) {
            throw new EvolutionApiError("Configuração da API inválida (URL, Chave ou Instância ausente).");
        }
        if (!jid) {
            throw new EvolutionApiError("Número (JID) é obrigatório para buscar histórico.");
        }
        ...
        // Alternative: Pass limit as query param if API supports it
        // const urlWithParams = `${url}?limit=${limit}`;

        console.log(`[EvolutionAPI] Fetching History: POST ${url}`);
        console.log(`  -> Payload:`, JSON.stringify(payload));

        try {
            const response = await fetch(url, { // or urlWithParams
                method: 'POST',
                headers: {
                    'apikey': apiConfig.apiKey,
                    'Content-Type': 'application/json'
            ...
            if (!Array.isArray(messagesArray)) {
                 console.error("[EvolutionAPI] Unexpected history response format (expected array):", responseData);
                 throw new EvolutionApiError("Formato de resposta do histórico inesperado.");
            }

            // Sort locally if API doesn't sort
            messagesArray.sort((a, b) => (a.messageTimestamp || 0) - (b.messageTimestamp || 0));

            return messagesArray as WhatsappMessage[];

        } catch (error: any) {
            console.error("[EvolutionAPI] Error in fetchChatHistory:", error);
            if (error instanceof EvolutionApiError) throw error;
